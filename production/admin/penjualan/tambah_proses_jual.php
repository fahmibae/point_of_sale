...
<body>
 <?php
            ...
            $tahun = date('Y-m-d');
            $cek_tahun = mysqli_query($konek, "SELECT * FROM barang WHERE kode_barang = '$kode_barang'");
            $data_tahun = mysqli_fetch_array($cek_tahun);
            $tahun_kadaluarsa = $data_tahun['tahun_kadaluarsa'];

            if ($tahun_kadaluarsa < $tahun){
              echo "
              <script type='text/javascript'>
              setTimeout(function () { 
              
                swal({
                        title: 'Mohon Maaf Tahun Barang Ini Sudah Kadaluarsa',
                        text:  '$kode_barang',
                        type: 'warning',
                        timer: 3000,
